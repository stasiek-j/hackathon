...
def run(cfg):
    ...
    st_dic = torch.load(cfg["ckpt"], map_location=device)['desc']
    # st_dic = {k.replace("module.", ""): v for k, v in st_dic.items()}
    model.load_state_dict(state_dict=st_dic)
    model.eval()
    if multi_gpu:
...
def main():
    ...
    parser.add_argument("--out-classes", type=int, default=0, help="number of output classes")
